...
import DellEMC from "assets/images/small-logos/dell-2.svg";
...
function Default() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/getNas").then((response) => {
      ...
      response.data.map((item) => {
        ...
        NetworkData.push({
          NodeSystem: (
            <Author image={DellEMC} name={item.ServerName} email={item.MangementIP} />
          ),
        ...
        <ArgonBox mb={3}>
          <Card>
            <ArgonBox
              ...
              p={3}
            >
              <ArgonTypography variant="h6">Node Nas</ArgonTypography>
            </ArgonBox>
            <ArgonBox
              sx={{
