...
function Default() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/getShipperGateway").then((response) => {
      ...
      response.data.map((item) => {
        ...
        NetworkData.push({
          NodeSystem: (
            <Author image={Gateway} name={item.ServerName} email={item.MangementIP} />
          ),
        ...
        <ArgonBox mb={3}>
          <Card>
            <ArgonBox
              ...
              p={3}
            >
              <ArgonTypography variant="h6">Node Shipper Gateway and Monitoring System</ArgonTypography>
            </ArgonBox>
            <ArgonBox
              sx={{
