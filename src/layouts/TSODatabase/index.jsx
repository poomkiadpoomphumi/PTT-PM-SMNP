...
import TSOdatabse from "assets/images/small-logos/database.png";
...
function Default() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/getTSODatabse").then((response) => {
      setColumns([
      ...
      response.data.map((item) => {
        ...
        NetworkData.push({
          NodeSystem: (
            <Author image={item.NodeType === 'Network Switch'? Cisco:TSOdatabse} name={item.ServerName} email={item.MangementIP} />
          ),
        ...
        <ArgonBox mb={3}>
          <Card>
            <ArgonBox
              ...
              p={3}
            >
              <ArgonTypography variant="h6">Node TSO Database Infrastructure</ArgonTypography>
            </ArgonBox>
            <ArgonBox
              sx={{
