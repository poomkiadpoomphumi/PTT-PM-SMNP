...
import Checkpoint from "assets/images/small-logos/checkpoint.png";
import Juniper from "assets/images/small-logos/Juniper.png";
...
function Default() {
  ...
  const CheckConnection = (Address, Brand) => {
    Axios.post(`http://localhost:8000/CheckConnection/${Address}`).then(
      (response) => {
        try {
          if (response.data === "error") {
            Swal.fire({
              icon: "error",
              title: "Oops...",
              text: Brand + " : Request timed out!",
              timer: 2000,
              showConfirmButton: false,
            });
          } else {
            window.location.href = `http://localhost:8001/views?address=${Address}&&servername=${Brand}`;
          }
        } catch (error) {
          console.error(error);
        }
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/getSwitchFireWall").then(
      (response) => {
      setColumns([
        ...
        NetworkData.push({
          ...
          Status: (
            <ArgonBadge
              ...
              color={item.Status == "online" ? "success" : 
                     item.Status == "ECONNREFUSED" ? "error" : "warning"}
              size="xs"
              container
            />
          ),
          ...
          action: (
            <ArgonTypography
              component="a"
              onClick={() => {
                CheckConnection(item.MangementIP, item.Brand);
              }}
              variant="caption"
              color="secondary"
              fontWeight="medium"
