...
import F5_Networks from "assets/images/small-logos/F5_Networks.svg";
...
function Default() {
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/getLoadBalance").then((response) => {
      ...
      response.data.map((item) => {
        ...
        NetworkData.push({
          NodeSystem: (
            <Author image={F5_Networks} name={item.ServerName} email={item.MangementIP} />
          ),
        ...
        <ArgonBox mb={3}>
          <Card>
            <ArgonBox
              ...
              p={3}
            >
              <ArgonTypography variant="h6">Node Load Balancer</ArgonTypography>
            </ArgonBox>
            <ArgonBox
              sx={{
